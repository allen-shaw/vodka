...
const (
	appVodka        = "vodka"
	appProtocGenGin = "protoc-gen-go-gin."
)

// 一些常量
const (
	commentTpl = `// Code generated by %v DO NOT EDIT.
// versions:
// 	%v       %v
// 	protoc      %v
// source: %v`
...
func GetComment(vodka bool, protocVersion string, source ...string) string {
	src := strings.Join(source, ",")
	app := appVodka
	if !vodka {
		app = appProtocGenGin
	}
	return fmt.Sprintf(commentTpl, app, app, version.Version, protocVersion, src)
}
...
func NewServiceGenerator(gen *protogen.Plugin, vodka bool, service *Service) *ServiceGenerator {
	...
	comment := GetComment(vodka, protocVersion, protocFiles...)
...
func NewRouterGenerator(gen *protogen.Plugin, vodka bool, s *Server) *RouterGenerator {
	...
	protocVersion := getProtocVersion(gen)
	protocFiles := getAllProtoFiles(s)
	comment := GetComment(vodka, protocVersion, protocFiles...)
...
func (g *RouterGenerator) execute() string {
	return execute(g, routerTpl)
}
...
func (g *ServerGenerator) execute() string {
	return execute(g, serverTpl)
}
...
func getAllProtoFiles(s *Server) []string {
	set := make(map[string]struct{}, 0)
	files := make([]string, 0)

	for _, svc := range s.Services {
		fn := *svc.ProtoFile.Proto.Name
		set[fn] = struct{}{}
	}
